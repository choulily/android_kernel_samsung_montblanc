...
static int bma255_i2c_read(struct i2c_client *client,
...
static int bma255_i2c_write(struct i2c_client *client,
		unsigned char reg_addr, unsigned char buf)
{
        int ret;
        struct i2c_msg msg;
...
static int bma255_set_mode(struct i2c_client *client, unsigned char mode)
{
	...
	ret = bma255_i2c_read(client, BMA255_MODE_CTRL_REG, &buf1);
	ret += bma255_i2c_read(client, BMA255_LOW_NOISE_CTRL_REG, &buf2);
	...
	case BMA255_MODE_NORMAL:
		...
		ret += bma255_i2c_write(client,	BMA255_MODE_CTRL_REG, buf1);
		mdelay(1);
		ret += bma255_i2c_write(client,
...
static int bma255_set_range(struct i2c_client *client, unsigned char range)
{
	...
	ret = bma255_i2c_read(client, BMA255_RANGE_SEL_REG, &buf);
	...
	ret += bma255_i2c_write(client, BMA255_RANGE_SEL_REG, buf);
		...
		unsigned char bandwidth)
{
	...
	ret = bma255_i2c_read(client, BMA255_BANDWIDTH__REG, &buf);
	buf = BMA255_SET_BITSLICE(buf, BMA255_BANDWIDTH, bandwidth);
	ret += bma255_i2c_write(client, BMA255_BANDWIDTH__REG, buf);
...
static int bma255_read_accel_xyz(struct bma255_p *data,	struct bma255_v *acc)
{
	...
	for (i = 0; i < READ_DATA_LENTH; i++) {
		ret += bma255_i2c_read(data->client,
		...
		unsigned char InterruptType , unsigned char value)
{
	...
	bma255_i2c_read(client, BMA255_INT_ENABLE1_REG, &reg);
	...
	bma255_i2c_write(client, BMA255_INT_ENABLE1_REG, reg);
}
		...
		int enable, int factory_mode)
{
	...
	if (enable == ON) {
		bma255_i2c_read(client, BMA255_EN_INT1_PAD_SLOPE__REG, &reg);
		reg = BMA255_SET_BITSLICE(reg, BMA255_EN_INT1_PAD_SLOPE, ON);
		bma255_i2c_write(client, BMA255_EN_INT1_PAD_SLOPE__REG, reg);

		bma255_i2c_read(client, BMA255_INT_MODE_SEL__REG, &reg);
		reg = BMA255_SET_BITSLICE(reg, BMA255_INT_MODE_SEL, 0x01);
		bma255_i2c_write(client, BMA255_INT_MODE_SEL__REG, reg);

		bma255_i2c_read(client, BMA255_SLOPE_DUR__REG, &reg);
		reg = BMA255_SET_BITSLICE(reg, BMA255_SLOPE_DUR,
				SLOPE_DURATION_VALUE);
		bma255_i2c_write(client, BMA255_SLOPE_DUR__REG, reg);

		if (factory_mode == OFF) {
			reg = SLOPE_THRESHOLD_VALUE;
			bma255_i2c_write(client, BMA255_SLOPE_THRES__REG, reg);
