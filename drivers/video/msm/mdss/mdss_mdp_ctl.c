				       ...
				       u32 *clk_rate)
{
	...
	*clk_rate = 0;
	...
	if (pipe_cnt == 1 /*&& backend_cnt == 0*/) {
		pr_debug("GPU fall-back case, ab/ib will be increased to 30 percent \n");
		ab_total = (ab_total*13)/10;
		ib_total = (ib_total*13)/10;
	}
	else if (pipe_cnt == 4 && backend_cnt == 4) { // 4layer bypass case
		pr_debug("4layer\n");
